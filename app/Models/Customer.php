...
//            'unrwa_help.required' => "الحقل مطلوب",
    ...
    function projects()
    {
        return $this->belongsToMany(Project::class, 'customer_projects', 'customer_id', 'project_id')->withPivot('print_id', 'coupon_no', 'tag','note');
    }
...
//        add family count
      if ($this->family_count_total) {
            if (intval($this->family_count_total) >= 7) {
                $points += 9;
            } else if (intval($this->family_count_total) < 5) {
                $points += 4;
            } else {
                $points += 7;
            }
