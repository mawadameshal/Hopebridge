...
class CustomerCopy extends Model
{
    //
//    use SoftDeletes;
    protected $table = 'customers_copy';
    protected $fillable = [
        ...
        'job_name', 'name_income', 'income_sum', 'outcome_sum',
        'house_owner', 'house_type', 'house_material',
        'researcher_opinion', 'researcher_rate','approval','main_id'
    ];
    ...
    function incomes()
    {
        return $this->hasMany(CustomerIncomeCopy::class, 'customer_id', 'id');
    }
    ...
    function outcomes()
    {
        return $this->belongsToMany(Outcome::class, 'customer_outcomes_copy', 'customer_id', 'outcome_id')->withPivot('amount');
    }

    function furnitures()
    {
        return $this->belongsToMany(Furniture::class, 'customer_furnitures_copy', 'customer_copy_id', 'furniture_id')->withPivot('count', 'rate');
    }
...
// additional helper relation for the count
    ...
    {
        return $this->belongsToMany(Furniture::class,'customer_furnitures_copy','customer_copy_id')
            ->withPivot('count', 'rate')
            ->selectRaw('sum(customer_furnitures_copy.rate) as aggregate')
            ->groupBy('customer_copy_id');
    }
...
//
    ...
    public static function validateData($request, $obj = "")
    {
        ...
        $rules = [
            'cus_name' => "required",
            'card_no' => "required|integer|unique:customers_copy,card_no," . ($obj ? $obj->id : ""),
...
//            'unrwa_help.required' => "الحقل مطلوب",
    ...
    {
        return $this->hasMany(\App\Models\AttachmentCopy::class);
    }

    public function needs()
    {
        return $this->hasMany(\App\Models\CustomerNeedCopy::class, 'customer_id', 'id');
    }

    public function getRegion()
    {
        return $this->belongsTo(Region::class, 'region_id')->withDefault(['name' => '_']);
    ...
    {
        return $this->hasMany(FamilyCopy::class, 'customer_copy_id', 'id');
    ...
    {
        return $this->belongsTo(User::class, 'updated_id')->withDefault(['name' => 'غير مدخل']);
    }

    public function approved()
    {
        return $this->belongsTo(User::class, 'approved_id')->withDefault(['name' => 'غير مدخل']);
    }
