...
//            (($request->get('citizin')) == 1 ? $mowaten : $ref)+
    ...
    public function getall_1(Request $request)
    {
        ...
        return DataTables::of(CustomerCopy::where('type', 1)
            ->with('getstate', 'getregion', 'gettype', 'user','getUpdated')->withCount('projects')->filter()
            ...
            ->addColumn('action', function ($query) {
                ...
                if (in_array(30, parent::$data["allowedActions"])) {
                    ...
                    if (!$check) {
                        $link = $link . '<a href="' . url('' . parent::$data["route"] . '/approve/' . $query->id . '') . '"  style="width: 60%;" class="btn btn-circle btn-icon-only btn-info approve_btn" title="موافقة">قبول اضافة</a>';
                    } else {
                        $link = $link . '<a href="' . url('' . parent::$data["route"] . '/approve/' . $query->id . '') . '" style="width: 60%;" class="btn btn-circle btn-icon-only btn-info approve_btn" title="موافقة">قبول تعديل</a>';
...
//
                ...
                if ($request['submit'] == 'accept') {
                    ...
                    $com = new Customer();
                    $com::create($customer_copy->toArray());

                    $incomes = [];
                    foreach ($customer_copy->incomes ?? [] as $key => $income) {
                        $incomes [] = [
                            'income_source' => $income['income_source'],
                            'income_type' => $income['income_type'] ?? '',
                            'income_amount' => $income['income_amount'] ?? '',
