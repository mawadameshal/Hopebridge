...
//            (($request->get('citizin')) == 1 ? $mowaten : $ref)+
    ...
    {
        return DataTables::of(Customer::where('type', 1)->where('approval_create',1)->with('getstate', 'getregion', 'gettype', 'user')->withCount('projects')->filter()
        )
    ...
    {
        return DataTables::of(Customer::where('type', 1)->where('approval_create',0)->with('getstate', 'getregion', 'gettype', 'user')->withCount('projects')->filter()
        )
            ->addColumn('action', function ($query) {
                $link = "";
                if (in_array(30, parent::$data["allowedActions"]))
                ...
                if (in_array(30, parent::$data["allowedActions"]))
                    $link = $link . '<a href="' . url('' . parent::$data["route"] . '/delete/' . $query->id . '') . '" class="btn btn-circle btn-icon-only btn-danger" id="delete_btn" title="حذف"><i class="fa fa-trash"></i> </a>';
                if (in_array(30, parent::$data["allowedActions"])) {
                    $link = $link . '<a href="' . url('' . parent::$data["route"] . '/approve/' . $query->id . '') . '" class="btn btn-circle btn-icon-only btn-info approve_btn" title="موافقة"><i class="fa fa-ban"></i> </a>';
                }
