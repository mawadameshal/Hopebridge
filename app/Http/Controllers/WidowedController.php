...
class WidowedController extends Controller
{
    ...
    public function index()
    {
        ...
        parent::$data['title'] = 'بحث عن السيدات الارامل';
    ...
    public function index_approval()
    {
        ...
        parent::getDataCounters();
        parent::$data['title'] = 'بحث عن السيدات الارامل الاحتياطيين';
        ...
        return view('widoweds.search_1', parent::$data);
    }
...
//            (($request->get('citizin')) == 1 ? $mowaten : $ref)+
    ...
    {
        return DataTables::of(Customer::where('type', 3)->where(function($query) {
            $query->where('approval_update',1)
                ->orWhere('approval_create',1);
        })->with('getstate', 'getregion', 'gettype', 'user')->withCount('projects')->filter()
        )
            ->addColumn('action', function ($query) {
                $link = "";
                if (in_array(30, parent::$data["allowedActions"]))
                    $link = $link . '<a href="Widowed/v1/' . $query->id . '/show"  class="btn btn-circle btn-icon-only btn-primary" title="عرض" id="show_btn"> <i class="fa fa-eye"></i> </a>';
                if (in_array(27, parent::$data["allowedActions"]))
                    $link = $link . '<a href="Widowed/v1/' . $query->id . '/edit"  class="btn btn-circle btn-icon-only btn-green" title="تعديل" id="edit_btn"> <i class="fa fa-edit"></i> </a>';
                if (in_array(30, parent::$data["allowedActions"]))
    ...
    {
        return DataTables::of(Customer::where('type', 3)->where(function($query) {
            $query->where('approval_update',0)
                ->orWhere('approval_create',0);
        })->with('getstate', 'getregion', 'gettype', 'user')->withCount('projects')->filter()
        )
            ->addColumn('action', function ($query) {
                $link = "";
    ...
    function approve_create($id, Request $request)
    {
        ...
        return redirect()->back()->with('msg', 'تم قبول الارملة بنجاح');
    }

    function approve_update($id, Request $request)
    {
        $customer = Customer::where('id', $id)->first();
        $customer->approval_update =  1;
        $customer->save();
        return redirect()->back()->with('msg', 'تم قبول تعديلات الارملة بنجاح');
    }

    function approve_delete($id, Request $request)
    {
        Customer::where('id', $id)->update(['approval_delete' => 1]);
        return redirect()->back()->with('msg', 'تم قبول حذف الارملة بنجاح');
    }
    ...
    public function show($id)
    {
        ...
        if (isset($customer)) {
            ...
            parent::$data['route'] = 'Widowed/show';
