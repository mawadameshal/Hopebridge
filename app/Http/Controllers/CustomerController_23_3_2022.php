...
//            (($request->get('citizin')) == 1 ? $mowaten : $ref)+
    ...
    {
        return DataTables::of(Customer::where(['type'=> 1,'id_update'=>NULL])->where('approval_create',1)->orWhere('approval_update',1)->with('getstate', 'getregion', 'gettype', 'user')->withCount('projects')->filter()
        )
    ...
    {
        return DataTables::of(Customer::where('type', 1)->where('approval_create',0)->orWhere('approval_update',0)->with('getstate', 'getregion', 'gettype', 'user')->withCount('projects')->filter()
        )
            ->addColumn('action', function ($query) {
                $link = "";
                if (in_array(30, parent::$data["allowedActions"]))
                    $link = $link . '<a href="Customer/v1/' . $query->id . '/show"  class="btn btn-circle btn-icon-only btn-primary" title="عرض" id="show_btn"> <i class="fa fa-eye"></i> </a>';
                if (in_array(30, parent::$data["allowedActions"])) {
                    if($query->approval_create == 0){
                        $link = $link . '<a href="' . url('' . parent::$data["route"] . '/approve_create/' . $query->id . '') . '"  style="width: 60%;" class="btn btn-circle btn-icon-only btn-info approve_btn" title="موافقة">قبول اضافة</a>';
                    }
                    else if($query->approval_create == 1 && $query->approval_update == 0){
                        $link = $link . '<a href="' . url('' . parent::$data["route"] . '/approve_update/' . $query->id . '') . '" style="width: 60%;" class="btn btn-circle btn-icon-only btn-info approve_btn" title="موافقة">قبول تعديل</a>';
    ...
    function approve_create($id, Request $request)
    {
        Customer::where('id', $id)->update(['approval_create' => 1,'approval_update' => 1]);
        return redirect()->back()->with('msg', 'تم قبول المستفيد بنجاح');
    }

    function approve_update($id, Request $request)
    {
        Customer::where('id', $id)->update(['approval_update' => 1]);
        return redirect()->back()->with('msg', 'تم قبول تعديلات المستفيد بنجاح');
    }

    function approve_delete($id, Request $request)
    {
        Customer::where('id', $id)->update(['approval_delete' => 1]);
        return redirect()->back()->with('msg', 'تم قبول حذف المستفيد بنجاح');
    }
...
//        dd($posts->get());
    ...
    public function needCustomerData(Request $request)
    {
        ...
        $child_no_to = $request->child_no_to;
        //$health = $request->health;
        $material = $request->material;
        $housetype = $request->housetype;
        $type = $request->type;
        $id = $request->customer_id;
        $id_to = $request->customer_id_to;

        $posts = Customer::with('gettype','getstate')->withCount('projects')->filter()
//            ->select('id','file_no','card_no', 'name','address','mobile','type','total_perc','child_no','total','state_id','region_id','house_material','house_type')
//            ->where(function ($q) use ($id,$id_to,$type,$name,$file_no,$card_no,$total,$total_to,$state,$region,$material,$housetype,$child_no,$child_no_to){
//                if(isset($id))
//                    $q->where('id','>=',$id);
//                if(isset($id_to))
//                    $q->where('id','<=',$id_to);
...
//                if(isset($file_no))
//                    $q->where('file_no','like',"%".$file_no."%");
//                if(isset($total))
//                    $q->where('total','>=',$total);
//                if(isset($total_to))
//                    $q->where('total','<=',$total_to);
//                if(isset($child_no))
//                    $q->where('child_no','>=',$child_no);
//                if(isset($child_no_to))
//                    $q->where('child_no','<=',$child_no_to);
//                if(isset($state))
//                    $q->where('state_id',$state);
//                if(isset($region))
//                    $q->where('region_id',$region);
//                if(isset($housetype))
//                    $q->where('house_type',$housetype);
//                if(isset($material))
//                    $q->where('house_material',$material);
//            })
//            ->leftjoin(DB::raw('(SELECT customer_id,project_id
//                FROM `customer_projects` where project_id ='.$project_id.')
//                cus'),
//                function($join)
//                {
//                    $join->on('customers.id', '=', 'cus.customer_id');
//                })
//            ->whereNull('cus.customer_id')
            ->orderBy('total', 'DESC');
