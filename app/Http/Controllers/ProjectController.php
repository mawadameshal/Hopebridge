...
class ProjectController extends Controller
{
    ...
    public function store(Request $request){
        if($request->ajax()){
            ...
            $obj=new Project();
            $obj->saveData($name,$status,$sponser,$desc);
    ...
    public function getAll(){
        ...
        return DataTables::of(Project::with('sponser'))
            ...
            ->addColumn('action',function ($query){
               ...
               if(in_array(17,parent::$data["allowedActions"])){
                   if($query->status == 1){
                       $link =$link.'<a href="' . url(''.parent::$data["route"].'/close/' . $query->id .'') . '" class="btn btn-primary btn-xs" id="close_btn" title="اغلاق">إغلاق </a>';
                   }else{
                       $link =$link.'<a href="' . url(''.parent::$data["route"].'/open/' . $query->id .'') . '" class="btn btn-primary btn-xs" id="open_btn" title="استرجاع">استرجاع</a>';
    ...
    public function close($id){
        ...
        if(isset($obj)){
           $obj->status = 2;
           $obj->save();
            return response(['status'=>true,'message'=>trans('ar.close_successfully'),201]);
        }
        return response(["status"=>false,"message"=>trans('ar.unsuccessful_state'),401]);
    }
    ...
    public function open($id){
        ...
        if(isset($obj)){
           $obj->status = 1;
           $obj->save();
            return response(['status'=>true,'message'=>trans('ar.open_successfully'),201]);
        }
        return response(["status"=>false,"message"=>trans('ar.unsuccessful_state'),401]);
    }
