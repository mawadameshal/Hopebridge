...
class YatemController extends Controller
{
    ...
    public function index_approval()
    {
        ...
        return view('yatem.search_1', parent::$data);
    ...
    {
        return DataTables::of(Orphan::where(function($query) {
            $query->where('approval_update',1)
                ->orWhere('approval_create',1);
        })->with('getstate', 'getregion', 'user')->withCount('projects')->filter()
        )
...
//            $obj->yatem_id = $yatem->id;
    ...
    function create_1()
    {
         ...
         if(session()->get('orphan_id')){
            session()->forget('orphan_id');
        }
...
//        $data = $request->only('researcher_opinion');
    ...
    {
        Orphan::where('id', $id)->update(['approval_create' => 1,'approval_update' => 1]);
        return redirect()->back()->with('msg', 'تم قبول اليتيم بنجاح');
    }

    function approve_update($id, Request $request)
    {
        Orphan::where('id', $id)->update(['approval_update' => 1]);
        return redirect()->back()->with('msg', 'تم قبول تعديلات اليتيم بنجاح');
    }
