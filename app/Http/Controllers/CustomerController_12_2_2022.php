...
//        parent::$data['customer']->calculatePoint();
    ...
    function personal($id = 0, Request $request)
    {
        ...
        $data = $request->only('name', 'state_id', 'researcher_name', 'region_id', 'neighborhood', 'address', 'street', 'mobile', 'mobile2','type');
...
//        return response(['status' => 'success', 'msg' => 'تم الحفظ بنجاح', 'id' => $id]);
    ...
    function fatherInfo($id = 0, Request $request)
    {
        ...
        $data = $request->only('father_name', 'birthday', 'card_no', 'card_no_wife', 'social_id', 'health_id', 'Handicap_type', 'disease', 'qualification_id');
        if (!$id)
            $data['stage'] = 2;
        else
            $data['updated_id'] = auth()->id();
...
//            (($request->get('citizin')) == 1 ? $mowaten : $ref)+
    ...
    {
        return DataTables::of(Customer::where('type', 1)->with('getstate', 'getregion', 'gettype', 'user')->withCount('projects')->filter()
        )
            ...
            if ($customer) {
                ...
                if ($message->save()) {
                    ...
                    curl_setopt_array($curl, array(
                        CURLOPT_URL => "https://www.tweetsms.ps/api.php?comm=sendsms&user=Jesr.Alamal&pass=258258&to=972$customer->mobile&message=$messageText&sender=Jesr.Alamal",
